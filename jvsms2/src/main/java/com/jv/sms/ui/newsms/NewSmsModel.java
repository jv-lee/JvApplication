...
public class NewSmsModel extends BaseModel implements NewSmsContract.Model {
     ...
     */
    @Override
    public Observable<List<ContactsBean>> findContactsAll() {
        return Observable.create(new ObservableOnSubscribe<List<ContactsBean>>() {
            @Override
     ...
     */
    @Override
    public Observable<List<LinkmanBean>> findLinkmanAll() {
        return Observable.create(new ObservableOnSubscribe<List<LinkmanBean>>() {
            @Override
    ...
    @Override
    public Observable<SmsBean> findLinkmanByPhoneNumber(String phoneNumber) {
        return Observable.just(phoneNumber).map(new Function<String, SmsBean>() {
            @Override
            public SmsBean apply(@NonNull String s) throws Exception {
                ...
                //查询raw_contacts表获得联系人id
                ContentResolver resolver = mApplication.getContentResolver();
                Cursor cur = resolver.query(Uri.parse("content://sms/"), new String[]{"_id", "address", "person", "body", "date", "type", "thread_id", "read"}, "address=?", new String[]{s}, "date desc limit 1");
                while (cur.moveToNext()) {
                    bean = new SmsBean();
                    bean.setPhoneNumber(s);
                    bean.setName(SmsUtil.getPeopleNameFromPerson(s, mApplication));
                    bean.setThread_id(cur.getString(cur.getColumnIndex("thread_id")));
                    bean.setColorPosition(Integer.parseInt(s.substring(s.length() - 1)) == 9 ? 0 : Integer.parseInt(s.substring(s.length() - 1)));
                }
                if (bean == null) {
                    bean = new SmsBean();
                    bean.setPhoneNumber(s);

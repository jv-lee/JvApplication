...
public class SystemUtils {
    public static int getKeyboardHeight(Activity paramActivity) {
        ...
        if (height == 0) {
            height = (int) SPHelper.get("KeyboardHeight", 787);//787为默认软键盘高度 基本差不离
        } else {
            SPHelper.save("KeyboardHeight", height);
        }
        return height;
    }
    ...
    public static void hideSoftInput(View paramEditText) {
        ((InputMethodManager) JvApplication.getInstance().getSystemService(Context.INPUT_METHOD_SERVICE))
    ...
    public static int dip2px(int dipValue) {
        float reSize = JvApplication.getInstance().getResources().getDisplayMetrics().density;
    ...
    public static void showKeyBoard(final View paramEditText) {
        ...
        paramEditText.post(new Runnable() {
            @Override
            public void run() {
                ((InputMethodManager) JvApplication.getInstance().getSystemService(Context.INPUT_METHOD_SERVICE)).showSoftInput(paramEditText, 0);
            }
        });
    }
}
